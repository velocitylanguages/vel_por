"""
Facebook Reels Automation - Bilingual English/Portuguese Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Portuguese branding
"""
...
# Portuguese translations for display
CATEGORIES_PORTUGUESE = {
    ...
    "Self Improvement": "Desenvolvimento Pessoal",
    "Gratitude": "Gratidão",
    "Friendship": "Amizade",
    "Hope": "Esperança",
...
PORTUGUESE_VOICE = "pt-BR-AntonioNeural"
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 2} unique {category_english} phrases for English speakers learning Portuguese.
...
2. Portuguese translation (with commas matching the rhythm)
...
IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before."""
            ...
            payload = {
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "You are a Portuguese teacher. Create short, natural phrases with pauses."},
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "portuguese": "Acredite em si mesmo.", "pronunciation": "Ah-kreh-jee-teh ehng see meh-ez-moh."},
            {"english": "You are capable of amazing things.", "portuguese": "Você é capaz de coisas incríveis.", "pronunciation": "Voh-seh eh kah-pahz jee koh-eez in-kree-veez."},
            {"english": "Dream big, start small.", "portuguese": "Sonhe grande, comece pequeno.", "pronunciation": "Soh-nyee grahn-jee, koh-meh-seh peh-keh-noh."},
            {"english": "Your future is created by your actions.", "portuguese": "Seu futuro é criado pelas suas ações.", "pronunciation": "Seh-oo foo-too-roh eh kree-ah-doh peh-lahs soo-ahs ah-soh-ees."},
            {"english": "Never give up on your dreams.", "portuguese": "Nunca desista dos seus sonhos.", "pronunciation": "Noon-kah jee-zeez-tah dohs seh-oos soh-nyohs."},
        ],
        "Love": [
            {"english": "Love yourself first.", "portuguese": "Ame-se primeiro.", "pronunciation": "Ah-mee see pree-may-roh."},
            {"english": "Love makes everything possible.", "portuguese": "O amor torna tudo possível.", "pronunciation": "Oo ah-mohr tohr-nah doo-doh poh-see-vee-oo."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        portuguese_file = output_dir / f"portuguese_{i}.mp3"
        combined_file = output_dir / f"combined_{i}.mp3"

        print(f"\n  Phrase {i+1}:")
        print(f"    EN: {phrase['english']}")
        print(f"    PT: {phrase['portuguese']}")
        ...
        pt_success = asyncio.run(generate_single_audio(phrase["portuguese"], PORTUGUESE_VOICE, str(portuguese_file)))
        if pt_success:
            print(f"    ✓ Portuguese: {portuguese_file.name}")
        else:
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "2", str(portuguese_file)]
            subprocess.run(cmd, capture_output=True)

        # Get ACTUAL durations
        en_duration = get_audio_duration(str(english_file))
        pt_duration = get_audio_duration(str(portuguese_file))

        # Add pause between English and Portuguese
        pause_between = 0.5
        total_duration = en_duration + pause_between + pt_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + PT: {pt_duration:.2f}s)")

        # Combine audio files
        cmd = [
            "ffmpeg", "-y",
            "-i", str(english_file),
            "-i", str(portuguese_file),
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{portuguese_file.as_posix()}'\n")
        ...
        audio_files.append({
            ...
            "portuguese": str(portuguese_file),
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    # Load fonts - Cross-platform with Unicode support for Portuguese characters
    # Windows fonts
    windows_fonts_bold = [
        "C:/Windows/Fonts/arialbd.ttf",  # Arial Bold
        "C:/Windows/Fonts/DejaVuSans-Bold.ttf",
    ]
    windows_fonts_regular = [
        "C:/Windows/Fonts/arial.ttf",  # Arial Regular
        "C:/Windows/Fonts/DejaVuSans.ttf",
    ]
    # Linux fonts (for GitHub Actions)
    linux_fonts_bold = [
    ...
    def find_font_path(fonts_bold, fonts_regular):
        import sys
        if sys.platform == "win32":
            for path in fonts_bold:
                if Path(path).exists():
                    return path
            for path in fonts_regular:
                if Path(path).exists():
                    return path
        for path in fonts_bold:
            if Path(path).exists():
    ...
    font_path_bold = find_font_path(windows_fonts_bold, linux_fonts_bold)
    font_path_regular = find_font_path(windows_fonts_regular, linux_fonts_regular)
    
    # Load fonts - Optimized for mobile viewing (INCREASED sizes)
    font_category = ImageFont.truetype(font_path_bold, 60)
    font_large = ImageFont.truetype(font_path_bold, 85)      # BOLD for English/Portuguese
    font_pronunciation = ImageFont.truetype(font_path_regular, 42)  # Regular for pronunciation
    font_branding = ImageFont.truetype(font_path_bold, 52)
    ...
    # Portuguese text
    portuguese_y = english_y + total_height + 110  # Increased from 100
    portuguese_lines = wrap_text(portuguese, font_large, VIDEO_WIDTH - 140)
    total_height = len(portuguese_lines) * 95  # Increased from 75

    draw.rectangle(
        [(60, portuguese_y - 55), (VIDEO_WIDTH - 60, portuguese_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(portuguese_lines):
        y_pos = portuguese_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = portuguese_y + total_height + 90  # Increased from 80
...
def create_video_from_images_audio(image_files: list, audio_files: list, combined_audio: str, output_file: str):
    ...
    for i, (img_path, audio_info) in enumerate(zip(image_files, audio_files)):
        ...
        print(f"  Image {i+1}/{len(image_files)}: {duration:.2f}s (EN: {audio_info.get('en_duration', 0):.1f}s + PT: {audio_info.get('pt_duration', 0):.1f}s)")
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_PORTUGUESE[category_english]})")
    print(f"{'='*80}\n")
    ...
    print("\n[3/4] Generating audio (English + Portuguese with 500ms pause)...")
    ...
    print(f"  🏷️  Branding: Velocity Portuguese")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇵🇹 VELOCITY PORTUGUESE - FACEBOOK REELS AUTOMATION 🇵🇹")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ Velocity Portuguese branding at bottom")
